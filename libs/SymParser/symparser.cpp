...
namespace symparser {
    namespace detail {
        std::filesystem::path get_cache_folder() {
            std::filesystem::path ret = KevlarGlobal::GetCacheDir();
    ...
    namespace {
        ...
        std::optional<std::filesystem::path> cache_pdb(structs::cv_pdb70_t* codeview) noexcept {
            ...
            std::string PdbName(codeview->pdb_name);
            std::string PdbNameLower = PdbName;
            for (auto& C : PdbNameLower) C = (char)tolower(C);
            static const char* KnownPdbs[] = {
                "ntkrnlmp.pdb", "ntoskrnl.pdb", "hal.pdb", "kd.pdb", "fltmgr.pdb",
                "ci.pdb", "cng.pdb", "ndis.pdb", "netio.pdb", "ntdll.pdb", "hidparse.pdb",
                "tcpip.pdb", "fwpkclnt.pdb", "wdf01000.pdb", "wdfldr.pdb", "clfs.pdb"
            };
            bool IsKnownPdb = false;
            for (auto& K : KnownPdbs) {
                if (PdbNameLower == K) { IsKnownPdb = true; break; }
            }
            if (!IsKnownPdb) {
                Logger::Log("{YEL}symparser: Skipping download of non-Microsoft PDB: %s{RESET}\n", codeview->pdb_name);
                return {};
            }
            ...
            Logger::Log("{CYN}symparser: Downloading %s{RESET}\n", codeview->pdb_name);
    ...
    std::vector<sym_t> download_symbols(const std::filesystem::path& img) {
        ...
        if (!pdb_path) {
            Logger::Log("{YEL}symparser: No PDB available for %s, continuing without symbols{RESET}\n", img_path_str.c_str());
            return std::vector<sym_t>();
        }
